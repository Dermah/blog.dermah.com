...
aws s3 ls --recursive s3://$BUCKET_NAME | tr -s ' ' | awk '{print $4 " " $3}' | sort > ./tmp/s3

# List all files and filesized in `dist/` to file
(find dist -type f -ls) | tr -s ' ' | awk '{system("ls -l " $11)}' | awk '{print $9 " " $5}' | sed 's&dist/&&' | sort > ./tmp/dist

# Word diff the two lists
# * S3 differences are red
# * Local differences are green
git diff --word-diff --no-index tmp/s3 tmp/dist || true